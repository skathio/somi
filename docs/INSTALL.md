...
repo-level utilities (`/atlas`, `/impact`, `/adopt`, `/pr`), and the lifecycle flows
...
`/review`, `/ship`, `/ship-loop`, `/plan-loop`, `/debug` (and the rest) in autocomplete. Try
`/plan list a trivial change` to confirm the planner agent loads. Then run `/plugin info somi` to
